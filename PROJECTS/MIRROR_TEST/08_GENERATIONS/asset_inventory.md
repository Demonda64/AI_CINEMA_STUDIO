# Inventaire des assets générés - Phase 3

Ce fichier suit les assets produits pour "Mirror".

## État actuel

Lot 01 généré et sélectionné. Cinq images sont retenues en première sélection.

## Dossiers

| Dossier | Usage | Statut |
|---------|-------|--------|
| `08_GENERATIONS/assets/images/drafts/` | Images brutes générées | Prêt |
| `08_GENERATIONS/assets/images/selected/` | Images retenues | Prêt |
| `08_GENERATIONS/assets/images/rejected/` | Images rejetées conservées | Prêt |
| `08_GENERATIONS/assets/video/drafts/` | Vidéos brutes générées | Prêt |
| `08_GENERATIONS/assets/video/selected/` | Vidéos retenues | Animatic lot 01 retenu |
| `08_GENERATIONS/assets/vendor_imports/heygen/` | Imports vidéo HeyGen | Prêt |
| `08_GENERATIONS/assets/vendor_imports/elevenlabs/` | Imports audio ElevenLabs | Prêt |
| `08_GENERATIONS/assets/vendor_imports/grok/` | Imports analyses/prompts Grok | Prêt |

## Assets attendus - Lot 01

| Asset ID | Type | Plan | Emplacement prévu | Statut |
|----------|------|------|-------------------|--------|
| MIRROR_S01_P02_IMAGE_v01 | Image | S01-P02 | `assets/images/drafts/` | Retenu |
| MIRROR_S02_P03_IMAGE_v01 | Image | S02-P03 | `assets/images/drafts/` | Retenu |
| MIRROR_S04_P01_IMAGE_v01 | Image | S04-P01 | `assets/images/drafts/` | À retoucher |
| MIRROR_S04_P01_IMAGE_v02 | Image | S04-P01 | `assets/images/drafts/` | Retenu |
| MIRROR_S04_P04_IMAGE_v01 | Image | S04-P04 | `assets/images/drafts/` | Retenu |
| MIRROR_S05_P02_IMAGE_v01 | Image | S05-P02 | `assets/images/drafts/` | Retenu |

## Assets retenus

| Asset ID | Type | Fichier | Utilisation | Notes |
|----------|------|---------|-------------|-------|
| MIRROR_S01_P02_IMAGE_v01 | Image | `08_GENERATIONS/assets/images/selected/MIRROR_S01_P02_IMAGE_v01.png` | Motif temporel | `3:17` lisible |
| MIRROR_S02_P03_IMAGE_v01 | Image | `08_GENERATIONS/assets/images/selected/MIRROR_S02_P03_IMAGE_v01.png` | Intrusion message | Texte exact |
| MIRROR_S04_P01_IMAGE_v02 | Image | `08_GENERATIONS/assets/images/selected/MIRROR_S04_P01_IMAGE_v02.png` | Première anomalie miroir | Reflet plus lisible |
| MIRROR_S04_P04_IMAGE_v01 | Image | `08_GENERATIONS/assets/images/selected/MIRROR_S04_P04_IMAGE_v01.png` | Révélation miroir | Reflet humain inquiétant |
| MIRROR_S05_P02_IMAGE_v01 | Image | `08_GENERATIONS/assets/images/selected/MIRROR_S05_P02_IMAGE_v01.png` | Fin ouverte | Présence subtile |

## Vidéos retenues

| Asset ID | Type | Fichier | Utilisation | Notes |
|----------|------|---------|-------------|-------|
| MIRROR_LOT01_ANIMATIC_v01 | Vidéo MP4 | `08_GENERATIONS/assets/video/selected/MIRROR_LOT01_ANIMATIC_v01.mp4` | Animatic lot 01 | Copie de `12_EXPORTS/MIRROR_TEST_animatic_lot01.mp4` |
| MIRROR_LOT01_ANIMATIC_AUDIO_v02 | Vidéo MP4 | `08_GENERATIONS/assets/video/selected/MIRROR_LOT01_ANIMATIC_AUDIO_v02.mp4` | Animatic lot 01 audio v02 | Copie de `12_EXPORTS/MIRROR_TEST_animatic_lot01_audio_v02.mp4` |
| MIRROR_LOT01_ANIMATIC_AUDIO_v03 | Vidéo MP4 | `08_GENERATIONS/assets/video/selected/MIRROR_LOT01_ANIMATIC_AUDIO_v03.mp4` | Animatic lot 01 audio v03 | Version recommandée pour revue |
